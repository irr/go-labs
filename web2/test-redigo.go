// go get -v github.com/hoisie/web
// curl -v "http://localhost:12345/root"
...
func checkError(err error, ctx *web.Context, msg string, status int) bool {
	if err != nil {
		log.Printf("%s: %s\n", msg, err.Error())
		http.Error(ctx.ResponseWriter, err.Error(), status)
	}
	return (err != nil)
}

func testServer(ctx *web.Context, user string) {
	...
	query, err := db.Query("SELECT Host FROM user WHERE User = ?", user)
	if checkError(err, ctx, "db.Query", http.StatusInternalServerError) {
		return
	}
	defer query.Close()
	...
	for query.Next() {
		...
		if checkError(err, ctx, "query.Scan", http.StatusInternalServerError) {
			return
		}
	...
	conn := rd.Get()
	if checkError(err, ctx, "rd.Get", http.StatusInternalServerError) {
		return
	}
	defer conn.Close()
	...
	*/
	...
	for n > 0 {
		if n == 1 {
			...
			if checkError(err, ctx, "conn.Receive(Strings)", http.StatusInternalServerError) {
				return
			}
			mr["redis"] = v
		} else {
			_, err := conn.Receive()
			if checkError(err, ctx, "conn.Receive(int64)", http.StatusInternalServerError) {
				return
			}
		}
		n--
	...
	if checkError(err, ctx, "json.Marshal", http.StatusInternalServerError) {
		return
	}
